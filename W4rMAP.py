...
class NmapGUI(QtWidgets.QMainWindow):
    def __init__(self):
        super().__init__()
    ...
    def init_ui(self):
        ...
        self.setStyleSheet("background-color: #210606; color: white;")

        # Load the logo from base64 and display it
        logo_base64 = '''
        ...
        self.ip_label = QtWidgets.QLabel('IP Addresses (comma-separated):')
        ...
        self.args_label = QtWidgets.QLabel('Nmap Command Line Arguments:')
        ...
        self.script_label = QtWidgets.QLabel('Nmap Script:')
        ...
        self.args_group = QtWidgets.QWidget()
        self.args_group.setLayout(QtWidgets.QVBoxLayout())
        ...
        self.scan_button = QtWidgets.QPushButton('Run Nmap Scan')
        ...
        self.thread_pool = QtCore.QThreadPool()  # Initialize the thread pool
    ...
    def browse_target_file(self):
        ...
        file_path, _ = QtWidgets.QFileDialog.getOpenFileName(self, 'Select Target File', '', 'Text Files (*.txt)',
                                                                   options=options)
        if file_path:
    ...
    def run_nmap_scan(self):
        ...
        try:
            ...
            selected_args = [arg for arg, label in self.args_checkboxes if any(
                cb.text() == label for cb in self.args_group.findChildren(QtWidgets.QCheckBox) if cb.isChecked())]
            ...
            def run_scan_task():
                ...
                while True:
                    ...
                    if not line:
                        break
                    line = line.rstrip()
                    QtCore.QMetaObject.invokeMethod(self.result_text, "append", QtCore.Qt.Q_ARG(str, line))
                    QtWidgets.QApplication.processEvents()

                process.communicate()
                if process.returncode != 0:
                    error_message = f"Process exited with error code: {process.returncode}"
                    QtCore.QMetaObject.invokeMethod(self.result_text, "append",
                                                          QtCore.Qt.Q_ARG(str, error_message))
            ...
            def run_scan_task():
                ...
                if process.returncode != 0:
                    error_message = f"Process exited with error code: {process.returncode}"
                    QtCore.QMetaObject.invokeMethod(self.result_text, "append", QtCore.Qt.Q_ARG(str, error_message))
